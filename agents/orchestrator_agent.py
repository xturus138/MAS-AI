...
class BridgePlan(BaseModel):
    bridge_steps: list = Field(
        description="Ordered list of navigation steps to move from the current screen to the required starting screen for the next scenario. Each step is a plain string instruction like 'click Back' or 'click Home button'."
    )
    reasoning: str = Field(
...
class FigmaMappingPlan(BaseModel):
    node_id: str = Field(
        description="The Figma Node ID (e.g. '10:5') of the frame that best matches the given test scenario menu/navigation context. Return an empty string if no suitable frame is found."
    )
...
class OrchestratorAgent:
    ...
    def _auto_discover_figma_node(self, menu_name: str, scenario_desc: str) -> Optional[str]:
        ...
        Returns the best-matching node ID, or None if no match is found."""
        if not self.figma or not self._mapping_llm:
            return None

        frames = self.figma.get_all_frames()
        if not frames:
            print("[Orchestrator] No frames returned from Figma, cannot auto-discover.")
            return None
        ...
        print(f"[Orchestrator] Auto-discovering Figma node for '{menu_name}'...")
        try:
            result = self._mapping_llm.invoke(messages)
            if result.node_id:
                print(f"[Orchestrator] Auto-discovered: '{result.frame_name}' ({result.node_id}) — {result.reasoning}")
                return result.node_id
            else:
                print(f"[Orchestrator] LLM could not find a matching frame for '{menu_name}'.")
                return None
        except Exception as e:
            print(f"[Orchestrator] Auto-discovery failed: {e}")
            return None
    ...
    def pre_scenario_discovery(self, scenario: dict, output_dir: str) -> dict:
        ...
        if not figma_enabled:
            print("[Figma] Figma integration disabled. Skipping discovery.")
            return {
                "figma_enabled": False,
                "figma_start_node_id": "",
                "figma_end_node_id": "",
        ...
        if not figma_start_node_id:
            print(f"[Figma] Could not resolve a Figma node for '{menu_name}'. Falling back to text-only validation.")
            return {
                "figma_enabled": False,
                "figma_start_node_id": "",
                "figma_end_node_id": "",
        ...
        if not figma_end_node_id:
            print("[Figma] WARN: Prototype path tracing failed, falling back to text-only validation")
            return {
                "figma_enabled": False,
    ...
    def compute_bridge(self, current_screen_description: str, next_start_node_id: str) -> list:
        ...
        print(f"[Orchestrator] Computing navigation bridge to '{next_screen_name}'...")
        try:
            result = self._bridge_llm.invoke(messages)
            print(f"[Orchestrator] Bridge plan: {result.bridge_steps}")
            return result.bridge_steps
        except Exception as e:
            print(f"[Orchestrator] Bridge computation failed: {e}")
            return []
