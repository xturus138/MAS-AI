...
def run_workflow():
    print("[*] Starting Scenario-Based Swarm System...")
    ...
    orchestrator = OrchestratorAgent(llm=orchestrator_llm, figma_adapter=figma_adapter)

    app = build_graph(observer, decider, executor, reflector, recorder, orchestrator)
    ...
    for scenario_index, target_scenario in enumerate(scenarios):
        ...
        if scenario_index < len(scenarios) - 1 and figma_adapter:
            ...
            if next_start_node:
                ...
                if bridge_steps:
                    print(f"[Orchestrator] Injecting {len(bridge_steps)} bridge step(s) into next scenario")
