...
Jangan simpan progress sesi di sini; progress → `IDEA.md`.
...
- **Stack inti:** Python 3.10+, LangGraph, LangChain, ADB / uiautomator2, Figma API, EasyOCR + OpenCV, MIRIX memory (SQLite+JSON), PyQt5 monitor
...
├── IDEA.md                ← status progress (sering diubah)
...
1. **Session start:** ikuti file ini; baca `IDEA.md` untuk status; untuk detail arsitektur baca `CLAUDE.md` (jangan salin ulang isinya ke sini).
...
8. **Progress berubah:** update `IDEA.md`. Jangan rewrite `AGENTS.md` untuk status harian.
9. **Scope memory:** konteks project lewat `AGENTS.md` + `IDEA.md` di folder ini (+ baca `CLAUDE.md` saat coding). Jangan cache body file ke fact_store global.
...
- **Observer:** screenshot → Canny+OCR → uiautomator XML + IoU matching widget; mode `OBSERVER_MODE` (`xml_first` / `pure_vision`).
...
- Status bug hari ini / blocker run terakhir → `IDEA.md`
